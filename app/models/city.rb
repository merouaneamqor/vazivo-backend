...
class City < ApplicationRecord
  ...
  before_validation :backfill_city_locales_from_canonical
  ...
  def backfill_city_locales_from_canonical
  ...
  def ensure_unique_slug_for_column(column, base)
    slug_candidate = base
    counter = 1
    while City.where(column => slug_candidate).where.not(id: id).exists?
